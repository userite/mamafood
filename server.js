...
const idService = require('./lib/identifier-service.js');
...
const initializeDatabase = async () => {
    ...
    try {
        ...
        // Таблица 8a: identifier_codes (генерирани идентификатори за QR/външни системи)
        try {
            await client.query(`
...
app.get('/', (req, res) => {
    res.json({ 
        ...
        endpoints: {
            ...
            identifiers: {
                catalog: 'GET /api/identifiers/catalog',
                generate: 'POST /api/identifiers/generate',
                store: 'POST /api/identifiers/store',
                validate: 'POST /api/identifiers/validate',
                parseQr: 'POST /api/identifiers/parse-qr',
                qrPng: 'POST /api/identifiers/qr',
                list: 'GET /api/identifiers/stored'
            }
        }
    });
});
...
// Винаги нов INSERT — без редактиране; за запис на вече генериран код от клиента
...
// Отделна страница за проверка на QR код (когато се сервира статично от този сървър)
